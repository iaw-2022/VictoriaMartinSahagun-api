...
const getComidasSinReservaByHuespedToken = async (req, res) => {
    try{
        ...
        await findId(info).then(
            async (huesped_id) => {
                ...
                const response = await db.query('SELECT * FROM comidas WHERE id NOT IN (SELECT comida_id FROM reservas_comidas WHERE cabana_id = $1)',[cabana.rows[0].cabana_id]);
...
module.exports = {
    ...
    getComidaById,
    getComidaByDay,
    getComidasSinReservaByHuespedToken
}