...
const getComidasSinReservaByHuespedId = async (req, res) => {
    ...
    if(!isNaN(id)){
        ...
        const comidas_reservadas = await db.query('SELECT comida_id FROM reservas_comidas WHERE cabana_id = $1',[cabana.rows[0].cabana_id]);
        const response = await db.query('SELECT * FROM comidas WHERE id!=$1',[comidas_reservadas.rows[0].comida_id]);
